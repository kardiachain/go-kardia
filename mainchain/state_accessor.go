...
func (k *KardiaService) stateAtTransaction(block *types.Block, txIndex int, reexec uint64) (blockchain.Message, kvm.BlockContext, *state.StateDB, error) {
	...
	for idx, tx := range block.Transactions() {
		...
		txContext := mkvm.NewEVMTxContext(msg)
		context := mkvm.NewEVMBlockContext(block.Header(), k.BlockChain(), nil)
		if idx == txIndex {
			return msg, context, statedb, nil
		}
		// Not yet the searched for transaction, execute on top of the current state
		vmenv := kvm.NewEVM(context, txContext, statedb, k.BlockChain().Config(), kvm.Config{})
