...
func (st *insertStats) report(chain []*types.Block, index int, dirty common.StorageSize) {
	...
	if index == len(chain)-1 || elapsed >= statsReportLimit {
		...
		context := []interface{}{
			...
			"number", end.Height(), "hash", end.Hash(),
		}
		if timestamp := time.Unix(end.Time().Unix(), 0); time.Since(timestamp) > time.Minute {
...
func newInsertIterator(chain types.Blocks, results <-chan error) *insertIterator {
...
func (it *insertIterator) next() (*types.Block, error) {
	...
	return it.chain[it.index], it.chain[it.index].ValidateBasic()
}
