...
func validateTxPoolInternals(pool *TxPool) error {
	...
	pool.priced.Reheap()
	priced, remote := pool.priced.remotes.Len(), pool.all.CountRemote()
	if priced != remote {
...
func (c *testChain) State() (*state.StateDB, error) {
	...
	stdb := c.statedb
	if *c.trigger {
		c.statedb, _ = state.New(nil, common.Hash{}, state.NewDatabase(memorydb.New()))
		// simulate that the new head block included tx0 and tx1
		c.statedb.SetNonce(c.address, 2)
		c.statedb.SetBalance(c.address, new(big.Int).SetUint64(configs.KAI))
		*c.trigger = false
	}
	return stdb, nil
}
...
func TestStateChangeDuringTransactionPoolReset(t *testing.T) {
	...
	statedb.SetBalance(address, new(big.Int).SetUint64(configs.KAI))
...
func TestTransactionNegativeValue(t *testing.T) {
	...
	tx, _ := types.SignTx(types.HomesteadSigner{}, types.NewTransaction(0, common.Address{}, big.NewInt(-1), 100, big.NewInt(1), nil), key)
	from, _ := deriveSender(tx)
	pool.currentState.AddBalance(from, big.NewInt(1))
...
func TestTransactionDoubleNonce(t *testing.T) {
	...
	tx1, _ := types.SignTx(types.HomesteadSigner{}, types.NewTransaction(0, common.Address{}, big.NewInt(100), 100000, big.NewInt(1), nil), key)
	tx2, _ := types.SignTx(types.HomesteadSigner{}, types.NewTransaction(0, common.Address{}, big.NewInt(100), 1000000, big.NewInt(2), nil), key)
	tx3, _ := types.SignTx(types.HomesteadSigner{}, types.NewTransaction(0, common.Address{}, big.NewInt(100), 1000000, big.NewInt(1), nil), key)
...
func TestTransactionGapFilling(t *testing.T) {
	...
	events := make(chan events.NewTxsEvent, testTxPoolConfig.AccountQueue+5)
...
func testTransactionQueueGlobalLimiting(t *testing.T, nolocals bool) {
	...
	for len(txs) < cap(txs) {
		key := keys[krand.Intn(len(keys)-1)] // skip adding transactions with the local account
		addr := crypto.PubkeyToAddress(key.PublicKey)
...
func TestTransactionAllowedTxSize(t *testing.T) {
	...
	if err := pool.addRemoteSync(pricedDataTransaction(1, pool.currentMaxGas, big.NewInt(1), key, uint64(krand.Intn(int(dataSize))))); err != nil {
	...
	if err := pool.addRemoteSync(pricedDataTransaction(2, pool.currentMaxGas, big.NewInt(1), key, dataSize+1+uint64(krand.Intn(int(10*txMaxSize))))); err == nil {
...
func TestTransactionPoolRepricing(t *testing.T) {
	...
	// Keep track of transaction events to ensure all executables get announced
	events := make(chan events.NewTxsEvent, 32)
	sub := pool.txFeed.Subscribe(events)
	defer sub.Unsubscribe()
