...
func runTrace(tracer *Tracer, vmctx *vmContext) (json.RawMessage, error) {
	env := kvm.NewKVM(vmctx.blockCtx, vmctx.txCtx, &dummyStatedb{}, kvm.Config{Debug: true, Tracer: tracer})
	var (
...
func TestTracer(t *testing.T) {
	execTracer := func(code string) []byte {
		t.Helper()
		ctx := &vmContext{blockCtx: kvm.Context{BlockHeight: big.NewInt(1)}, txCtx: kvm.TxContext{GasPrice: big.NewInt(100000)}}
...
func TestHaltBetweenSteps(t *testing.T) {
	...
	env := kvm.NewKVM(kvm.Context{BlockHeight: big.NewInt(1)}, kvm.TxContext{}, &dummyStatedb{}, kvm.Config{Debug: true, Tracer: tracer})
