...
// pushBigInt create a JavaScript BigInteger in the kvm.
...
func New(code string, txCtx kvm.TxContext) (*Tracer, error) {
	tracer := &Tracer{
		kvm:             duktape.New(),
	...
	tracer.kvm.PushGlobalGoFunction("isPrecompiled", func(ctx *duktape.Context) int {
		_, ok := kvm.PrecompiledContractsV0[common.BytesToAddress(popSlice(ctx))]
		ctx.PushBoolean(ok)
		return 1
	})
	...
	if !tracer.kvm.GetPropString(tracer.tracerObject, "step") {
...
func (jst *Tracer) call(method string, args ...string) (json.RawMessage, error) {
	...
	return json.RawMessage(jst.kvm.JsonEncode(-1)), nil
}
...
func (jst *Tracer) CaptureStart(env *kvm.KVM, from common.Address, to common.Address, create bool, input []byte, gas uint64, value *big.Int) {
	...
	intrinsicGas, err := tx_pool.IntrinsicGas(input, to.Equal(common.Address{}))
	if err != nil {
		return
	}
...
func (jst *Tracer) GetResult() (json.RawMessage, error) {
	...
	for key, val := range jst.ctx {
		switch val := val.(type) {
		case uint64:
			jst.kvm.PushUint(uint(val))

		case string:
			jst.kvm.PushString(val)

		case []byte:
			ptr := jst.kvm.PushFixedBuffer(len(val))
			copy(makeSlice(ptr, uint(len(val))), val)

		case common.Address:
			ptr := jst.kvm.PushFixedBuffer(20)
			copy(makeSlice(ptr, 20), val[:])

		case *big.Int:
			pushBigInt(val, jst.kvm)
