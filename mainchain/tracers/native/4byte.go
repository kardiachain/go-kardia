...
// 4 byte-identifiers of a tx, and implements kvm.KVMLogger.
...
func (t *fourByteTracer) CaptureStart(env *kvm.KVM, from common.Address, to common.Address, create bool, input []byte, gas uint64, value *big.Int) {
	t.env = env

	// Update list of precompiles based on current block
	t.activePrecompiles = kvm.ActivePrecompiles()
...
func (t *fourByteTracer) CaptureState(pc uint64, op kvm.OpCode, gas, cost uint64, scope *kvm.ScopeContext, rData []byte, depth int, err error) {
}

// CaptureEnter is called when KVM enters a new scope (via call, create or selfdestruct).
func (t *fourByteTracer) CaptureEnter(op kvm.OpCode, from common.Address, to common.Address, input []byte, gas uint64, value *big.Int) {
	...
	if op != kvm.DELEGATECALL && op != kvm.STATICCALL &&
		op != kvm.CALL && op != kvm.CALLCODE {
		return
	}
...
func (t *fourByteTracer) CaptureFault(pc uint64, op kvm.OpCode, gas, cost uint64, scope *kvm.ScopeContext, depth int, err error) {
}
