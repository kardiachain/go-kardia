...
func TestZeroValueToNotExitCall(t *testing.T) {
	...
	signer := types.HomesteadSigner{}
	tx, err := types.SignTx(signer, types.NewTransaction(0, to, big.NewInt(0), 500000, big.NewInt(0), nil), privkey)
	if err != nil {
		t.Fatalf("err %v", err)
	}
	origin, _ := signer.Sender(tx)
	...
	var code = []byte{
		byte(kvm.PUSH1), 0x0, byte(kvm.DUP1), byte(kvm.DUP1), byte(kvm.DUP1), // in and outs zero
		byte(kvm.DUP1), byte(kvm.PUSH1), 0xff, byte(kvm.GAS), // value=0,address=0xff, gas=GAS
		byte(kvm.CALL),
	}
	...
	st := blockchain.NewStateTransition(env, msg, new(types.GasPool).AddGas(tx.Gas()))
	if _, err = st.TransitionDb(); err != nil {
		t.Fatalf("failed to execute transaction: %v", err)
	}
	// Retrieve the trace result and compare against the etalon
