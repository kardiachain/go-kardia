...
func TestEviction(t *testing.T) {
	...
	var id uint64
	_ = db.Update(ctx, func(tx kvstore.RwTx) error {
		_ = tx.Put(kvstore.PlainState, k1[:], []byte{1})
...
func TestAPI(t *testing.T) {
	...
	get := func(key [20]byte, expectTxnID uint64) (res [1]chan []byte) {
		...
		for i := 0; i < len(res); i++ {
			...
			go func(out chan []byte) {
				require.NoError(db.View(context.Background(), func(tx kvstore.Tx) error {
	...
	put := func(k, v []byte) uint64 {
		var txID uint64
		require.NoError(db.Update(context.Background(), func(tx kvstore.RwTx) error {
			_ = tx.Put(kvstore.PlainState, k, v)
			txID = tx.ViewID()
			return nil
		}))
		return txID
	...
	err := db.View(context.Background(), func(tx kvstore.Tx) error {
