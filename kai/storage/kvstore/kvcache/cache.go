...
import (
	...
	gointerfaces "github.com/kardiachain/go-kardia/proto/kardiachain"
...
type Cache interface {
	// View - returns CacheView consistent with givent kvstore.Tx
	View(ctx context.Context, tx kvstore.Tx) (CacheView, error)
...
//   - method View can't parent.Clone() - because parent view is not coherent with current kvstore.Tx
//   - only OnNewBlock method may do parent.Clone() and apply StateChanges to create coherent view of kvstore.Tx
...
func (c *Coherent) View(ctx context.Context, tx kvstore.Tx) (CacheView, error) {
	id := ViewID(tx.ViewID())
...
func (c *Coherent) Get(k []byte, tx kvstore.Tx, id ViewID) ([]byte, error) {
	...
	v, err := tx.GetOne(kvstore.PlainState, k)
	if err != nil {
		return nil, err
	}
	...
	c.lock.Lock()
	defer c.lock.Unlock()
	v = c.add(common.CopyBytes(k), common.CopyBytes(v), r, id).V
	return v, nil
}

func (c *Coherent) GetCode(k []byte, tx kvstore.Tx, id ViewID) ([]byte, error) {
	...
	v, err := tx.GetOne(kvstore.Code, k)
	if err != nil {
		return nil, err
	}
	//fmt.Printf("from db: %#x,%x\n", k, v)
	...
	c.lock.Lock()
	defer c.lock.Unlock()
	v = c.addCode(common.CopyBytes(k), common.CopyBytes(v), r, id).V
	return v, nil
}
...
func AssertCheckValues(ctx context.Context, tx kvstore.Tx, cache Cache) (int, error) {
	...
	root.cache.Ascend(func(i *Element) bool {
		k, v := i.K, i.V
		var dbV []byte
		dbV, err = tx.GetOne(kvstore.PlainState, k)
		if err != nil {
			return false
		}
