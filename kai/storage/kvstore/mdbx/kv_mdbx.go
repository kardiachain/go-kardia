...
const NonExistingDBI kvstore.DBI = 999_999_999

type TableCfgFunc func(defaultBuckets kvstore.TableCfg) kvstore.TableCfg

func WithChaindataTables(defaultBuckets kvstore.TableCfg) kvstore.TableCfg {
...
type MdbxOpts struct {
	...
	path          string
	inMem         bool
	label         kvstore.Label // marker to distinct db instances - one process may open many databases. for example to collect metrics of only 1 database
	verbosity     kvstore.DBVerbosityLvl
...
func NewMDBX(log log.Logger) MdbxOpts {
	return MdbxOpts{
		...
		log:        log,
		pageSize:   kvstore.DefaultPageSize(),
		growthStep: 2 * datasize.GB,
	}
}

func (opts MdbxOpts) GetLabel() kvstore.Label { return opts.label }
func (opts MdbxOpts) GetInMem() bool          { return opts.inMem }
func (opts MdbxOpts) GetPageSize() uint64     { return opts.pageSize }

func (opts MdbxOpts) Label(label kvstore.Label) MdbxOpts {
	opts.label = label
	return opts
}
...
func (opts MdbxOpts) DBVerbosity(v kvstore.DBVerbosityLvl) MdbxOpts {
	opts.verbosity = v
	return opts
}
...
func (opts MdbxOpts) Open() (kvstore.RwDB, error) {
	var err error
	if opts.inMem {
	...
	if err = env.SetOption(mdbx.OptMaxReaders, kvstore.ReadersLimit); err != nil {
		return nil, err
	}
	...
	db := &MdbxKV{
		...
		log:          opts.log,
		wg:           &sync.WaitGroup{},
		buckets:      kvstore.TableCfg{},
		txSize:       dirtyPagesLimit * opts.pageSize,
		roTxsLimiter: opts.roTxsLimiter,
	}

	customBuckets := opts.bucketsCfg(kvstore.ChaindataTablesCfg)
...
func (opts MdbxOpts) MustOpen() kvstore.RwDB {
	db, err := opts.Open()
	if err != nil {
...
func (db *MdbxKV) openDBIs(buckets []string) error {
	if db.opts.flags&mdbx.Readonly != 0 {
		if err := db.View(context.Background(), func(tx kvstore.Tx) error {
			for _, name := range buckets {
				if db.buckets[name].IsDeprecated {
					continue
				}
				if err := tx.(kvstore.BucketMigrator).CreateBucket(name); err != nil {
					return err
				}
			}
		...
		if err := db.Update(context.Background(), func(tx kvstore.RwTx) error {
			for _, name := range buckets {
				if db.buckets[name].IsDeprecated {
					continue
				}
...
func (db *MdbxKV) BeginRo(ctx context.Context) (txn kvstore.Tx, err error) {
	if db.closed.Load() {
...
func (db *MdbxKV) BeginRw(_ context.Context) (txn kvstore.RwTx, err error) {
	if db.closed.Load() {
		return nil, fmt.Errorf("db closed")
	}
...
type MdbxTx struct {
	...
	statelessCursors map[string]kvstore.Cursor
	readOnly         bool
	cursorID         uint64
}
...
type MdbxCursor struct {
	...
	c          *mdbx.Cursor
	bucketName string
	bucketCfg  kvstore.TableCfgItem
	dbi        mdbx.DBI
	id         uint64
}
...
func (db *MdbxKV) AllDBI() map[string]kvstore.DBI {
	res := map[string]kvstore.DBI{}
...
func (db *MdbxKV) AllBuckets() kvstore.TableCfg {
	return db.buckets
}
...
func (tx *MdbxTx) CollectMetrics() {
	if tx.db.opts.label != kvstore.ChainDB {
		return
	}
	...
	kvstore.DbSize.Set(info.Geo.Current)
	kvstore.DbPgopsNewly.Set(info.PageOps.Newly)
	kvstore.DbPgopsCow.Set(info.PageOps.Cow)
	kvstore.DbPgopsClone.Set(info.PageOps.Clone)
	kvstore.DbPgopsSplit.Set(info.PageOps.Split)
	kvstore.DbPgopsMerge.Set(info.PageOps.Merge)
	kvstore.DbPgopsSpill.Set(info.PageOps.Spill)
	kvstore.DbPgopsUnspill.Set(info.PageOps.Unspill)
	kvstore.DbPgopsWops.Set(info.PageOps.Wops)
	kvstore.DbPgopsGcrtime.Update(info.PageOps.Gcrtime.Seconds())
	...
	kvstore.TxDirty.Set(txInfo.SpaceDirty)
	kvstore.TxLimit.Set(tx.db.txSize)
	kvstore.TxSpill.Set(txInfo.Spill)
	kvstore.TxUnspill.Set(txInfo.Unspill)

	gc, err := tx.BucketStat("gc")
	if err != nil {
		return
	}
	kvstore.GcLeafMetric.Set(gc.LeafPages)
	kvstore.GcOverflowMetric.Set(gc.OverflowPages)
	kvstore.GcPagesMetric.Set((gc.LeafPages + gc.OverflowPages) * tx.db.opts.pageSize / 8)
}
...
func (db *MdbxKV) View(ctx context.Context, f func(tx kvstore.Tx) error) (err error) {
	if db.closed.Load() {
...
func (db *MdbxKV) Update(ctx context.Context, f func(tx kvstore.RwTx) error) (err error) {
	if db.closed.Load() {
		return fmt.Errorf("db closed")
	}
...
func (tx *MdbxTx) CreateBucket(name string) error {
	...
	if err == nil {
		...
		cnfCopy.Flags = kvstore.TableFlags(flags)
	...
	if flags&kvstore.DupSort != 0 {
		nativeFlags |= mdbx.DupSort
		flags ^= kvstore.DupSort
	}
	if flags != 0 {
...
func (tx *MdbxTx) DropBucket(bucket string) error {
	if cfg, ok := tx.db.buckets[bucket]; !(ok && cfg.IsDeprecated) {
		return fmt.Errorf("%w, bucket: %s", kvstore.ErrAttemptToDeleteNonDeprecatedBucket, bucket)
	}
...
func (tx *MdbxTx) Commit() error {
	...
	if tx.db.opts.label == kvstore.ChainDB {
		kvstore.DbCommitPreparation.Update(latency.Preparation.Seconds())
		kvstore.DbCommitGc.Update(latency.GC.Seconds())
		kvstore.DbCommitAudit.Update(latency.Audit.Seconds())
		kvstore.DbCommitWrite.Update(latency.Write.Seconds())
		kvstore.DbCommitSync.Update(latency.Sync.Seconds())
		kvstore.DbCommitEnding.Update(latency.Ending.Seconds())
		kvstore.DbCommitTotal.Update(latency.Whole.Seconds())
	}
...
func (tx *MdbxTx) statelessCursor(bucket string) (kvstore.RwCursor, error) {
	if tx.statelessCursors == nil {
		tx.statelessCursors = make(map[string]kvstore.Cursor)
	}
...
func (tx *MdbxTx) IncrementSequence(bucket string, amount uint64) (uint64, error) {
	c, err := tx.statelessCursor(kvstore.Sequence)
	if err != nil {
		return 0, err
	}
...
func (tx *MdbxTx) RwCursor(bucket string) (kvstore.RwCursor, error) {
	b := tx.db.buckets[bucket]
	if b.AutoDupSortKeysConversion {
		return tx.stdCursor(bucket)
	}

	if b.Flags&kvstore.DupSort != 0 {
...
func (tx *MdbxTx) Cursor(bucket string) (kvstore.Cursor, error) {
	return tx.RwCursor(bucket)
}

func (tx *MdbxTx) stdCursor(bucket string) (kvstore.RwCursor, error) {
	b := tx.db.buckets[bucket]
...
func (tx *MdbxTx) RwCursorDupSort(bucket string) (kvstore.RwCursorDupSort, error) {
...
func (tx *MdbxTx) CursorDupSort(bucket string) (kvstore.CursorDupSort, error) {
	return tx.RwCursorDupSort(bucket)
}
...
func bucketSlice(b kvstore.TableCfg) []string {
