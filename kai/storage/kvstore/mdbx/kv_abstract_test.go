...
func TestSequence(t *testing.T) {
	...
	writeDBs, _ := setupDatabases(t, log.New(), func(defaultBuckets kvstore.TableCfg) kvstore.TableCfg {
	...
	for _, db := range writeDBs {
		...
		i, err := tx.ReadSequence(kvstore.ChaindataTables[0])
		require.NoError(t, err)
		require.Equal(t, uint64(0), i)
		i, err = tx.IncrementSequence(kvstore.ChaindataTables[0], 1)
		require.NoError(t, err)
		require.Equal(t, uint64(0), i)
		i, err = tx.IncrementSequence(kvstore.ChaindataTables[0], 6)
		require.NoError(t, err)
		...
		i, err = tx.ReadSequence(kvstore.ChaindataTables[1])
		require.NoError(t, err)
		require.Equal(t, uint64(0), i)
		i, err = tx.IncrementSequence(kvstore.ChaindataTables[1], 1)
		require.NoError(t, err)
		require.Equal(t, uint64(0), i)
		i, err = tx.IncrementSequence(kvstore.ChaindataTables[1], 6)
		require.NoError(t, err)
		require.Equal(t, uint64(1), i)
		i, err = tx.IncrementSequence(kvstore.ChaindataTables[1], 1)
		require.NoError(t, err)
...
func TestManagedTx(t *testing.T) {
	...
	logger := log.New()
	defaultConfig := kvstore.ChaindataTablesCfg
	defer func() {
		kvstore.ChaindataTablesCfg = defaultConfig
	}()

	bucketID := 0
	bucket1 := kvstore.ChaindataTables[bucketID]
	bucket2 := kvstore.ChaindataTables[bucketID+1]
	writeDBs, readDBs := setupDatabases(t, logger, func(defaultBuckets kvstore.TableCfg) kvstore.TableCfg {
		return map[string]kvstore.TableCfgItem{
			bucket1: {
...
func setupDatabases(t *testing.T, logger log.Logger, f mdbx.TableCfgFunc) (writeDBs []kvstore.RwDB, readDBs []kvstore.RwDB) {
	t.Helper()
	writeDBs = []kvstore.RwDB{
...
func testMultiCursor(t *testing.T, db kvstore.RwDB, bucket1, bucket2 string) {
	t.Helper()
	assert, ctx := assert.New(t), context.Background()

	if err := db.View(ctx, func(tx kvstore.Tx) error {
