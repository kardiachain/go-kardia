...
func BaseCase(t *testing.T) (kvstore.RwDB, kvstore.RwTx, kvstore.RwCursorDupSort) {
	t.Helper()
	path := t.TempDir()
	logger := log.New()
	table := "Table"
	db := NewMDBX(logger).Path(path).WithTableCfg(func(defaultBuckets kvstore.TableCfg) kvstore.TableCfg {
		return kvstore.TableCfg{
			table:            kvstore.TableCfgItem{Flags: kvstore.DupSort},
			kvstore.Sequence: kvstore.TableCfgItem{},
		}
	}).MustOpen()
...
func iteration(t *testing.T, c kvstore.RwCursorDupSort, start []byte, val []byte) ([]string, []string) {
	t.Helper()
	var keys []string
	var values []string
	var err error
...
func baseAutoConversion(t *testing.T) (kvstore.RwDB, kvstore.RwTx, kvstore.RwCursor) {
	t.Helper()
	path := t.TempDir()
	logger := log.New()
	...
	c, err := tx.RwCursor(kvstore.PlainState)
	require.NoError(t, err)
...
func TestAutoConversionSeekBothRange(t *testing.T) {
	...
	c, err := tx.RwCursorDupSort(kvstore.PlainState)
	require.NoError(t, err)
