...
import (
	...
	"github.com/kardiachain/go-kardia/kai/storage/kvstore/memdb"
...
func TestBucketCRUD(t *testing.T) {
	...
	normalBucket := kvstore.ChaindataTables[15]
	deprecatedBucket := kvstore.ChaindataDeprecatedTables[0]
	migrator, ok := tx.(kvstore.BucketMigrator)
	if !ok {
		return
	}

	// check thad buckets have unique DBI's
	uniquness := map[kvstore.DBI]bool{}
	...
	require.True(errors.Is(migrator.DropBucket(normalBucket), kvstore.ErrAttemptToDeleteNonDeprecatedBucket))
...
func TestReadOnlyMode(t *testing.T) {
	path := t.TempDir()
	logger := log.New()
	db1 := mdbx.NewMDBX(logger).Path(path).WithTableCfg(func(defaultBuckets kvstore.TableCfg) kvstore.TableCfg {
		return kvstore.TableCfg{
			kvstore.Headers: kvstore.TableCfgItem{},
		}
	}).MustOpen()
	db1.Close()

	db2 := mdbx.NewMDBX(logger).Readonly().Path(path).WithTableCfg(func(defaultBuckets kvstore.TableCfg) kvstore.TableCfg {
		return kvstore.TableCfg{
			kvstore.Headers: kvstore.TableCfgItem{},
		}
	}).MustOpen()
	...
	c, err := tx.Cursor(kvstore.Headers)
	require.NoError(t, err)
