...
type MemoryMutation struct {
	...
	db               kvstore.Tx
	statelessCursors map[string]kvstore.RwCursor
}
...
// batch.Commit()
func NewMemoryBatch(tx kvstore.Tx) *MemoryMutation {
...
func (m *MemoryMutation) UpdateTxn(tx kvstore.Tx) {
	m.db = tx
...
func initSequences(db kvstore.Tx, memTx kvstore.RwTx) error {
	cursor, err := db.Cursor(kvstore.Sequence)
	if err != nil {
		return err
	}
	for k, v, err := cursor.First(); k != nil; k, v, err = cursor.Next() {
		...
		if err := memTx.Put(kvstore.Sequence, k, v); err != nil {
			return err
		}
	}
	return nil
...
func (m *MemoryMutation) statelessCursor(table string) (kvstore.RwCursor, error) {
	if m.statelessCursors == nil {
		m.statelessCursors = make(map[string]kvstore.RwCursor)
	}
...
func (m *MemoryMutation) Flush(tx kvstore.RwTx) error {
...
func isTablePurelyDupsort(bucket string) bool {
	config, ok := kvstore.ChaindataTablesCfg[bucket]
	// If we do not have the configuration we assume it is not dupsorted
	if !ok {
		return false
	}
	return !config.AutoDupSortKeysConversion && config.Flags == kvstore.DupSort
}

// Cursor creates a new cursor (the real fun begins here)
func (m *MemoryMutation) makeCursor(bucket string) (kvstore.RwCursorDupSort, error) {
...
// Cursor creates a new cursor (the real fun begins here)
func (m *MemoryMutation) RwCursorDupSort(bucket string) (kvstore.RwCursorDupSort, error) {
	return m.makeCursor(bucket)
}

// Cursor creates a new cursor (the real fun begins here)
func (m *MemoryMutation) RwCursor(bucket string) (kvstore.RwCursor, error) {
	return m.makeCursor(bucket)
}

// Cursor creates a new cursor (the real fun begins here)
func (m *MemoryMutation) CursorDupSort(bucket string) (kvstore.CursorDupSort, error) {
	return m.makeCursor(bucket)
}

// Cursor creates a new cursor (the real fun begins here)
func (m *MemoryMutation) Cursor(bucket string) (kvstore.Cursor, error) {
	return m.makeCursor(bucket)
}
