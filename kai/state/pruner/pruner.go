...
func prune(snaptree *snapshot.Tree, root common.Hash, maindb kaidb.Database, stateBloom *stateBloom, bloomPath string, middleStateRoots map[common.Hash]struct{}, start time.Time) error {
...
func (p *Pruner) Prune(root common.Hash) error {
	...
	if root == (common.Hash{}) {
		...
		layers = p.snaptree.Snapshots(p.chainHeader.AppHash, 128, true)
		if len(layers) != 128 {
	...
	if !rawdb.HasLegacyTrieNode(p.db, root) {
		...
		if len(layers) > 0 {
			log.Info("Selecting bottom-most difflayer as the pruning target", "root", root, "height", p.chainHeader.Height-127)
		} else {
...
// in the disk.
func RecoverPruning(datadir string, db kaidb.Database, trieCachePath string) error {
...
func extractGenesis(db kaidb.Database, stateBloom *stateBloom) error {
