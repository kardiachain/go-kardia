...
func TestDiskMerge(t *testing.T) {
	...
	kvstore.WriteAccountSnapshot(db, accNoModNoCache, accNoModNoCache[:])
	kvstore.WriteAccountSnapshot(db, accNoModCache, accNoModCache[:])
	kvstore.WriteAccountSnapshot(db, accModNoCache, accModNoCache[:])
	kvstore.WriteAccountSnapshot(db, accModCache, accModCache[:])
	kvstore.WriteAccountSnapshot(db, accDelNoCache, accDelNoCache[:])
	kvstore.WriteAccountSnapshot(db, accDelCache, accDelCache[:])

	kvstore.WriteAccountSnapshot(db, conNoModNoCache, conNoModNoCache[:])
	kvstore.WriteStorageSnapshot(db, conNoModNoCache, conNoModNoCacheSlot, conNoModNoCacheSlot[:])
	kvstore.WriteAccountSnapshot(db, conNoModCache, conNoModCache[:])
	kvstore.WriteStorageSnapshot(db, conNoModCache, conNoModCacheSlot, conNoModCacheSlot[:])
	kvstore.WriteAccountSnapshot(db, conModNoCache, conModNoCache[:])
	kvstore.WriteStorageSnapshot(db, conModNoCache, conModNoCacheSlot, conModNoCacheSlot[:])
	kvstore.WriteAccountSnapshot(db, conModCache, conModCache[:])
	kvstore.WriteStorageSnapshot(db, conModCache, conModCacheSlot, conModCacheSlot[:])
	kvstore.WriteAccountSnapshot(db, conDelNoCache, conDelNoCache[:])
	kvstore.WriteStorageSnapshot(db, conDelNoCache, conDelNoCacheSlot, conDelNoCacheSlot[:])
	kvstore.WriteAccountSnapshot(db, conDelCache, conDelCache[:])
	kvstore.WriteStorageSnapshot(db, conDelCache, conDelCacheSlot, conDelCacheSlot[:])

	kvstore.WriteAccountSnapshot(db, conNukeNoCache, conNukeNoCache[:])
	kvstore.WriteStorageSnapshot(db, conNukeNoCache, conNukeNoCacheSlot, conNukeNoCacheSlot[:])
	kvstore.WriteAccountSnapshot(db, conNukeCache, conNukeCache[:])
	kvstore.WriteStorageSnapshot(db, conNukeCache, conNukeCacheSlot, conNukeCacheSlot[:])

	kvstore.WriteSnapshotRoot(db, baseRoot)
	...
	assertDatabaseAccount := func(account common.Hash, data []byte) {
		t.Helper()
		if blob := kvstore.ReadAccountSnapshot(db, account); !bytes.Equal(blob, data) {
	...
	assertDatabaseStorage := func(account common.Hash, slot common.Hash, data []byte) {
		t.Helper()
		if blob := kvstore.ReadStorageSnapshot(db, account, slot); !bytes.Equal(blob, data) {
	...
	for i := 0; i < 1024; i++ {
		...
		insertAccount := func(account common.Hash, data []byte) {
			if bytes.Compare(account[:], genMarker) <= 0 {
				kvstore.WriteAccountSnapshot(db, account, data[:])
			}
		}
		...
		insertStorage := func(account common.Hash, slot common.Hash, data []byte) {
			if bytes.Compare(append(account[:], slot[:]...), genMarker) <= 0 {
				kvstore.WriteStorageSnapshot(db, account, slot, data[:])
			}
		}
		...
		assertDatabaseAccount := func(account common.Hash, data []byte) {
			t.Helper()
			blob := kvstore.ReadAccountSnapshot(db, account)
		...
		assertDatabaseStorage := func(account common.Hash, slot common.Hash, data []byte) {
			t.Helper()
			blob := kvstore.ReadStorageSnapshot(db, account, slot)
...
func TestDiskGeneratorPersistence(t *testing.T) {
	...
	kvstore.WriteAccountSnapshot(db, accOne, accOne[:])
	kvstore.WriteStorageSnapshot(db, accOne, accOneSlotOne, accOneSlotOne[:])
	kvstore.WriteStorageSnapshot(db, accOne, accOneSlotTwo, accOneSlotTwo[:])
	kvstore.WriteSnapshotRoot(db, baseRoot)
	...
	blob := kvstore.ReadSnapshotGenerator(db)
...
func TestDiskSeek(t *testing.T) {
	...
	if dir, err := ioutil.TempDir("", "disklayer-test"); err != nil {
		...
		db = storage.NewDatabase(diskdb)
	}
	// Fill even keys [0,2,4...]
	for i := 0; i < 0xff; i += 2 {
		acc := common.Hash{byte(i)}
		kvstore.WriteAccountSnapshot(db, acc, acc[:])
	}
	// Add an 'higher' key, with incorrect (higher) prefix
	highKey := []byte{kvstore.SnapshotAccountPrefix[0] + 1}
