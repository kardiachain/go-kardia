...
func loadAndParseLegacyJournal(db kaidb.KeyValueStore, base *diskLayer) (snapshot, journalGenerator, error) {
	...
	// in progress only.
	journal := kvstore.ReadSnapshotJournal(db)
	if len(journal) == 0 {
...
func loadAndParseJournal(db kaidb.KeyValueStore, base *diskLayer) (snapshot, journalGenerator, error) {
	...
	// layer is invalid.
	generatorBlob := kvstore.ReadSnapshotGenerator(db)
...
func loadSnapshot(diskdb kaidb.KeyValueStore, triedb *trie.TrieDatabase, cache int, root common.Hash, recovery bool) (snapshot, error) {
	// Retrieve the block number and hash of the snapshot, failing if no snapshot
	// is present in the database (or crashed mid-update).
	baseRoot := kvstore.ReadSnapshotRoot(diskdb)
