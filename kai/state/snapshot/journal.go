...
func loadAndParseJournal(db kaidb.KeyValueStore, base *diskLayer) (snapshot, journalGenerator, error) {
	...
	// not existent, e.g. the disk layer is generating while that the Kaigo
...
func loadSnapshot(diskdb kaidb.KeyValueStore, triedb *trie.Database, root common.Hash, cache int, recovery bool, noBuild bool) (snapshot, bool, error) {
	...
	//
	// Possible scenario: Kaigo was crashed without persisting journal and then
...
func iterateJournal(db kaidb.KeyValueReader, callback journalCallback) error {
