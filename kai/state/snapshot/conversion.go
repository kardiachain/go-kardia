...
type (
	...
	trieGeneratorFn func(db kaidb.KeyValueWriter, in chan (trieKV), out chan (common.Hash))
...
func GenerateTrie(snaptree *Tree, root common.Hash, src kaidb.Database, dst kaidb.KeyValueWriter) error {
	...
	got, err := generateTrieRoot(dst, acctIt, common.Hash{}, stackTrieGenerate, func(dst kaidb.KeyValueWriter, accountHash, codeHash common.Hash, stat *generateStats) (common.Hash, error) {
		// Migrate the code first, commit the contract code into the tmp db.
		if codeHash != emptyCode {
			code := trie.ReadCode(src, codeHash)
			if len(code) == 0 {
				return common.Hash{}, errors.New("failed to read contract code")
			}
			trie.WriteCode(dst, codeHash, code)
		}
...
func generateTrieRoot(db kaidb.KeyValueWriter, it Iterator, account common.Hash, generatorFn trieGeneratorFn, leafCallback leafCallbackFn, stats *generateStats, report bool) (common.Hash, error) {
	var (
...
func stackTrieGenerate(db kaidb.KeyValueWriter, in chan trieKV, out chan common.Hash) {
