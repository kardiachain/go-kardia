...
type Tree struct {
	diskdb kaidb.KeyValueStore      // Persistent database to store the snapshot
	triedb *trie.TrieDatabase       // In-memory cache to access the trie through
...
func New(diskdb kaidb.KeyValueStore, triedb *trie.TrieDatabase, cache int, root common.Hash, async bool, rebuild bool, recovery bool) (*Tree, error) {
...
func diffToDisk(bottom *diffLayer) *diskLayer {
	...
	kvstore.DeleteSnapshotRoot(batch)
	...
	for hash := range bottom.destructSet {
		...
		kvstore.DeleteAccountSnapshot(batch, hash)
		base.cache.Set(hash[:], nil)

		it := kvstore.IterateStorageSnapshots(base.diskdb, hash)
		for it.Next() {
	...
	for hash, data := range bottom.accountData {
		...
		kvstore.WriteAccountSnapshot(batch, hash, data)
	...
	for accountHash, storage := range bottom.storageData {
		...
		for storageHash, data := range storage {
			...
			if len(data) > 0 {
				kvstore.WriteStorageSnapshot(batch, accountHash, storageHash, data)
				base.cache.Set(append(accountHash[:], storageHash[:]...), data)
				snapshotCleanStorageWriteMeter.Mark(int64(len(data)))
			} else {
				kvstore.DeleteStorageSnapshot(batch, accountHash, storageHash)
	...
	kvstore.WriteSnapshotRoot(batch, bottom.root)
...
func (t *Tree) Journal(root common.Hash) (common.Hash, error) {
	...
	kvstore.WriteSnapshotJournal(t.diskdb, journal.Bytes())
	return base, nil
}
...
func (t *Tree) Rebuild(root common.Hash) {
	...
	kvstore.DeleteSnapshotRecoveryNumber(t.diskdb)
...
func (t *Tree) Verify(root common.Hash) error {
	...
	got, err := generateTrieRoot(nil, acctIt, common.Hash{}, stackTrieGenerate, func(db kaidb.KeyValueWriter, accountHash, codeHash common.Hash, stat *generateStats) (common.Hash, error) {
