...
func CheckDanglingStorage(chaindb kaidb.KeyValueStore) error {
...
func checkDanglingDiskStorage(chaindb kaidb.KeyValueStore) error {
	var (
		lastReport = time.Now()
		start      = time.Now()
		lastKey    []byte
...
// snapshot difflayers.
func checkDanglingMemStorage(db kaidb.KeyValueStore) error {
	start := time.Now()
...
func CheckJournalAccount(db kaidb.KeyValueStore, hash common.Hash) error {
