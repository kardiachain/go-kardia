...
func generateSnapshot(diskdb kaidb.KeyValueStore, triedb *trie.TrieDatabase, cache int, root common.Hash, wiper chan struct{}) *diskLayer {
	...
	kvstore.WriteSnapshotRoot(batch, root)
...
func journalProgress(db kaidb.KeyValueWriter, marker []byte, stats *generatorStats) {
	...
	kvstore.WriteSnapshotGenerator(db, blob)
}
...
func (dl *diskLayer) generate(stats *generatorStats) {
	...
	for accIt.Next() {
		...
		if accMarker == nil || !bytes.Equal(accountHash[:], accMarker) {
			kvstore.WriteAccountSnapshot(batch, accountHash, data)
		...
		if batch.ValueSize() > kaidb.IdealBatchSize || abort != nil {
		...
		if acc.Root != emptyRoot {
			...
			for storeIt.Next() {
				kvstore.WriteStorageSnapshot(batch, accountHash, common.BytesToHash(storeIt.Key), storeIt.Value)
