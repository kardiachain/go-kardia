...
func ReadSnapshotDisabled(db kaidb.KeyValueReader) bool {
	disabled, _ := db.Has(snapshotDisabledKey)
	return disabled
}

// WriteSnapshotDisabled stores the snapshot pause flag.
func WriteSnapshotDisabled(db kaidb.KeyValueWriter) {
...
func DeleteSnapshotDisabled(db kaidb.KeyValueWriter) {
...
func ReadSnapshotRoot(db kaidb.KeyValueReader) common.Hash {
...
// the persisted snapshot.
func WriteSnapshotRoot(db kaidb.KeyValueWriter, root common.Hash) {
...
// invalid.
func DeleteSnapshotRoot(db kaidb.KeyValueWriter) {
...
func ReadAccountSnapshot(db kaidb.KeyValueReader, hash common.Hash) []byte {
	data, _ := db.Get(accountSnapshotKey(hash))
	return data
}

// WriteAccountSnapshot stores the snapshot entry of an account trie leaf.
func WriteAccountSnapshot(db kaidb.KeyValueWriter, hash common.Hash, entry []byte) {
...
func DeleteAccountSnapshot(db kaidb.KeyValueWriter, hash common.Hash) {
...
func ReadStorageSnapshot(db kaidb.KeyValueReader, accountHash, storageHash common.Hash) []byte {
	data, _ := db.Get(storageSnapshotKey(accountHash, storageHash))
	return data
}

// WriteStorageSnapshot stores the snapshot entry of an storage trie leaf.
func WriteStorageSnapshot(db kaidb.KeyValueWriter, accountHash, storageHash common.Hash, entry []byte) {
...
func DeleteStorageSnapshot(db kaidb.KeyValueWriter, accountHash, storageHash common.Hash) {
...
func IterateStorageSnapshots(db kaidb.Iteratee, accountHash common.Hash) kaidb.Iterator {
...
func ReadSnapshotJournal(db kaidb.KeyValueReader) []byte {
...
func WriteSnapshotJournal(db kaidb.KeyValueWriter, journal []byte) {
...
// the last shutdown
func DeleteSnapshotJournal(db kaidb.KeyValueWriter) {
...
// the last shutdown.
func ReadSnapshotGenerator(db kaidb.KeyValueReader) []byte {
...
// shutdown.
func WriteSnapshotGenerator(db kaidb.KeyValueWriter, generator []byte) {
...
// the last shutdown
func DeleteSnapshotGenerator(db kaidb.KeyValueWriter) {
...
// snapshot layer.
func ReadSnapshotRecoveryNumber(db kaidb.KeyValueReader) *uint64 {
...
// snapshot layer.
func WriteSnapshotRecoveryNumber(db kaidb.KeyValueWriter, number uint64) {
	var buf [8]byte
...
// snapshot layer.
func DeleteSnapshotRecoveryNumber(db kaidb.KeyValueWriter) {
...
func ReadSnapshotSyncStatus(db kaidb.KeyValueReader) []byte {
	data, _ := db.Get(snapshotSyncStatusKey)
	return data
}

// WriteSnapshotSyncStatus stores the serialized sync status to save at shutdown.
func WriteSnapshotSyncStatus(db kaidb.KeyValueWriter, status []byte) {
