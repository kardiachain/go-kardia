...
import (
	...
	libcommon "github.com/kardiachain/go-kardia/lib/common"
...
// !Important: [from, to)
func TruncateRange(db kvstore.RwTx, bucket string, key []byte, to uint32) error {
	...
	return WalkChunkWithKeys(key, bm, ChunkLimit, func(chunkKey []byte, chunk *roaring.Bitmap) error {
		...
		return db.Put(bucket, chunkKey, libcommon.CopyBytes(buf.Bytes()))
	})
}

// Get - reading as much chunks as needed to satisfy [from, to] condition
// join all chunks to 1 bitmap by Or operator
func Get(db kvstore.Tx, bucket string, key []byte, from, to uint32) (*roaring.Bitmap, error) {
...
// starts from hot shard, stops when shard not overlap with [from-to)
// !Important: [from, to)
func TruncateRange64(db kvstore.RwTx, bucket string, key []byte, to uint64) error {
...
// Get - reading as much chunks as needed to satisfy [from, to] condition
// join all chunks to 1 bitmap by Or operator
func Get64(db kvstore.Tx, bucket string, key []byte, from, to uint64) (*roaring64.Bitmap, error) {
