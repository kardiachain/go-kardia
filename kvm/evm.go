...
func NewEVM(blockCtx BlockContext, txCtx TxContext, statedb StateDB, chainConfig *configs.ChainConfig, config Config) *EVM {
	evm := &EVM{
		Context:     blockCtx,
		TxContext:   txCtx,
		StateDB:     statedb,
...
func (evm *EVM) CreateGenesisContractAddress(caller ContractRef, code []byte, gas uint64, value *big.Int, genesisContractAddr common.Address) (err error) {
	_, _, _, vmerr := evm.create(caller, &codeAndHash{code: code}, gas, value, genesisContractAddr, CREATE)
	if vmerr != nil {
		return vmerr
	}
	return nil
