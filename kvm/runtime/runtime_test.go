...
// func (s *stepCounter) CaptureFault(pc uint64, op vm.OpCode, gas, cost uint64, scope *kvm.ScopeContext, depth int, err error) {
// }

// func (s *stepCounter) CaptureEnd(output []byte, gasUsed uint64, t time.Duration, err error) {}

// func (s *stepCounter) CaptureState(pc uint64, op vm.OpCode, gas, cost uint64, scope *kvm.ScopeContext, rData []byte, depth int, err error) {
// 	s.steps++
