...
echo "Download custom VM startup script from Google Storage"
curl https://storage.googleapis.com/kardia-startup-scripts/ec2_user_data.txt > /tmp/ec2_user_data.txt
...
echo "Generate new SSH keypair stored at ./kardiavm-key.pem"
aws ec2 create-key-pair --key-name kardiavm-key --query 'KeyMaterial' --output text > kardiavm-key.pem
chmod 400 kardiavm-key.pem

echo "Create new EC2"
EC_ID=`aws ec2 run-instances --image-id $AMI_ID --security-group-ids kardia-sg --count 1 --instance-type t2.medium --block-device-mapping DeviceName=/dev/xvda,Ebs={VolumeSize=30} --key-name kardiavm-key --tag-specifications 'ResourceType=instance,Tags=[{Key=Name,Value=kardia-testnet}]' --user-data file:///tmp/ec2_user_data.txt --query 'Instances[0].InstanceId'`
