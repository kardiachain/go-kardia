...
func keyLookup(evidence types.Evidence) []byte {
	return keyLookupFromHeightAndHash(evidence.Height().Int64(), evidence.Hash().Bytes())
}
...
func keyOutqueue(evidence types.Evidence, priority int64) []byte {
	return _key("%s/%s/%s/%X", baseKeyOutqueue, bE(priority), bE(evidence.Height().Int64()), evidence.Hash())
}

func keyPending(evidence types.Evidence) []byte {
	return _key("%s/%s/%X", baseKeyPending, bE(evidence.Height().Int64()), evidence.Hash())
}
...
func (store *Store) getInfo(evidence types.Evidence) Info {
	return store.GetInfo(evidence.Height().Int64(), evidence.Hash().Bytes())
}
