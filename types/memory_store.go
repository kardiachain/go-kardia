...
func (db *MemStore) WriteEvent(smc *KardiaSmartcontract) {
	...
	// Write contract abi
	smcObj := smartContract{
		Address: smc.SmcAddress,
		ABI:     smc.SmcAbi,
	}
	encodedData, err := rlp.EncodeToBytes(smcObj)
	if err != nil {
	...
	if err := db.Put(contractAbiKey(masterSmc.Address), encodedData); err != nil {
...
func (db *MemStore) ReadEvent(address string, method string) *Watcher {
...
func (db *MemStore) ReadEvents(address string) (string, []*Watcher) {
	...
	if err := rlp.DecodeBytes(data.([]byte), &events); err != nil {
		log.Error("Invalid event lookup rlp", "err", err)
		return "", nil
	}

	watcherActions := make([]*Watcher, 0)
...
func (db *memBatch) ReadEvent(address string, method string) *Watcher {
	data, err := db.Get(eventKey(address, method))
	if err != nil {
...
func (db *memBatch) ReadEvents(address string) (string, []*Watcher) {
