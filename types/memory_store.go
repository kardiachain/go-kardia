...
func (db *MemStore) WriteEvent(smc *KardiaSmartcontract) {
	address := common.HexToAddress(smc.SmcAddress)
...
func (db *MemStore) ReadEvent(address *common.Address, method string) *WatcherAction {
	data, err := db.Get(eventKey(address, method))
	if err != nil {
		log.Error("error while get event", "err", err, "address", address.Hex(), "method", method)
		return nil
	}
...
func (db *MemStore) ReadSmartContractFromDualAction(action string) (*common.Address, *abi.ABI) {
...
func (db *MemStore) ReadSmartContractAbi(address *common.Address) *abi.ABI {
	data, err := db.Get(contractAbiKey(address))
	if err != nil || data == nil {
		log.Error("error while get abi from contract address", "err", err, "address", address.Hex())
		return nil
	}
	abiStr := string(data.([]byte))
	a, err := abi.JSON(strings.NewReader(abiStr))
	if err != nil {
		log.Error("cannot get abi from smart contract", "err", err, "address", address.Hex())
		return nil
	}
	return &a
}
...
func (db *memBatch) ReadEvent(address *common.Address, method string) *WatcherAction {
	data, err := db.Get(eventKey(address, method))
	if err != nil {
		log.Error("error while get event", "err", err, "address", address.Hex(), "method", method)
		return nil
...
func (db *memBatch) ReadSmartContractFromDualAction(action string) (*common.Address, *abi.ABI) {
	key, err := db.Get(dualActionKey(action))
	if err != nil || key == nil {
		return nil, nil
	}
...
func (db *memBatch) ReadSmartContractAbi(address *common.Address) *abi.ABI {
	data, err := db.Get(contractAbiKey(address))
	if err != nil || data == nil {
		log.Error("error while get abi from contract address", "err", err, "address", address.Hex())
		return nil
