...
func New(diskdb kaidb.Database, cleans *fastcache.Cache, resolver ChildResolver) *Database {
	return &Database{
		diskdb:   diskdb,
		resolver: resolver,
		cleans:   cleans,
...
func (db *Database) commit(hash common.Hash, batch kaidb.Batch, uncacher *cleaner) error {
	// If the node does not exist, it's a previously committed node
	node, ok := db.dirties[hash]
	if !ok {
		return nil
