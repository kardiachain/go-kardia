...
func NewDatabase(diskdb kaidb.KeyValueStore) *TrieDatabase {
...
func NewDatabaseWithConfig(diskdb kaidb.KeyValueStore, config *Config) *TrieDatabase {
...
func (db *TrieDatabase) DiskDB() kaidb.KeyValueStore {
	return db.diskdb
}
...
func (db *TrieDatabase) insert(hash common.Hash, size int, node node) {
...
func (db *TrieDatabase) node(hash common.Hash) node {
...
func (db *TrieDatabase) Node(hash common.Hash) ([]byte, error) {
	...
	// Content unavailable in memory, attempt to retrieve from disk
	enc := ReadTrieNode(db.diskdb, hash)
	if len(enc) != 0 {
		if db.cleans != nil {
			db.cleans.Set(hash[:], enc)
			memcacheCleanMissMeter.Mark(1)
...
func (db *TrieDatabase) preimage(hash common.Hash) []byte {
	// Short circuit if preimage collection is disabled
	if db.preimages == nil {
		return nil
	}
	...
	return ReadPreimage(db.diskdb, hash)
}
...
func (db *TrieDatabase) Cap(limit common.StorageSize) error {
	...
	for size > limit && oldest != (common.Hash{}) {
		...
		WriteTrieNode(batch, oldest, node.rlp())
...
// Note, this method is a non-synchronized mutator. It is unsafe to call this
// concurrently with other mutators.
func (db *TrieDatabase) Commit(node common.Hash, report bool, callback func(common.Hash)) error {
	// Create a database batch to flush persistent data out. It is important that
	// outside code doesn't see an inconsistent state (referenced data removed from
	// memory cache during commit but not yet in persistent storage). This is ensured
	// by only uncaching existing data when the database write finalizes.
...
func (db *TrieDatabase) commit(hash common.Hash, batch kaidb.Batch, uncacher *cleaner, callback func(common.Hash)) error {
...
func (c *cleaner) Put(key []byte, rlp []byte) error {
	...
	case c.db.oldest:
		...
		c.db.nodes[node.flushNext].flushPrev = common.Hash{}
	case c.db.newest:
		c.db.newest = node.flushPrev
		c.db.nodes[node.flushPrev].flushNext = common.Hash{}
	default:
		c.db.nodes[node.flushPrev].flushNext = node.flushNext
		c.db.nodes[node.flushNext].flushPrev = node.flushPrev
	}
	// Remove the node from the dirty cache
	delete(c.db.nodes, hash)
	c.db.nodesSize -= common.StorageSize(common.HashLength + int(node.size))
	if node.children != nil {
		c.db.nodesSize -= common.StorageSize(cachedNodeChildrenSize + len(node.children)*(common.HashLength+2))
	}
...
func (db *TrieDatabase) Size() (common.StorageSize, common.StorageSize) {
	db.lock.RLock()
	defer db.lock.RUnlock()
...
func (db *TrieDatabase) SaveCachePeriodically(dir string, interval time.Duration, stopCh <-chan struct{}) {
