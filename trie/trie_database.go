...
//
// Note, the trie TrieDatabase is **not** thread safe in its mutations, but it **is**
// thread safe in providing individual, independent node access. The rationale
// behind this split design is to provide read access to RPC handlers and sync
// servers even while the trie is executing expensive garbage collection.
type TrieDatabase struct {
	diskdb kaidb.KeyValueStore // Persistent storage for matured trie nodes
...
func NewDatabaseWithCache(diskdb kaidb.Database, cache int) *TrieDatabase {
...
func (db *TrieDatabase) InsertBlob(hash common.Hash, blob []byte) {
	db.lock.Lock()
...
func (db *TrieDatabase) commit(hash common.Hash, batch kaidb.Batch, uncacher *cleaner) error {
