...
func (t *Trie) Prove(key []byte, fromLevel uint, proofDb kaidb.KeyValueWriter) error {
...
// nodes of the longest existing prefix of the key (at least the root node), ending
// with the node that proves the absence of the key.
func (t *StateTrie) Prove(key []byte, fromLevel uint, proofDb kaidb.KeyValueWriter) error {
...
func proofToPath(rootHash common.Hash, root node, key []byte, proofDb kaidb.KeyValueReader, allowNonExistent bool) (node, []byte, error) {
...
func VerifyRangeProof(rootHash common.Hash, firstKey []byte, lastKey []byte, keys [][]byte, values [][]byte, proof kaidb.KeyValueReader) (bool, error) {
