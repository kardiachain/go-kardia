...
func (sc *scheduler) setPeerRange(peerID p2p.ID, base uint64, height uint64) error {
...
func (sc *scheduler) getPeersWithHeight(height uint64) []p2p.ID {
	peers := make([]p2p.ID, 0)
...
func (sc *scheduler) prunablePeers(peerTimeout time.Duration, minRecvRate int64, now time.Time) []p2p.ID {
...
func (sc *scheduler) markReceived(peerID p2p.ID, height uint64, size uint64, now time.Time) error {
...
func (sc *scheduler) markPending(peerID p2p.ID, height uint64, time time.Time) error {
...
func (sc *scheduler) pendingFrom(peerID p2p.ID) []uint64 {
	var heights []uint64
	for height, pendingPeerID := range sc.pendingBlocks {
		if pendingPeerID == peerID {
...
func (sc *scheduler) selectPeer(height uint64) (p2p.ID, error) {
	...
	for mp := range pendingFrom {
		if uint64(mp) < minPending {
			minPending = uint64(mp)
		}
	}
