		...
		Subcommands: []*cli.Command{
			{
				...
				Description: `
kaigo snapshot prune-state <state-root>
...
during the use of Kaigo, please also specify it here for correct deletion. Otherwise
...
`,
			},
			{
				...
				Description: `
kaigo snapshot verify-state <state-root>
...
`,
			},
			{
				...
				Flags:     flags.Merge(utils.NetworkFlags, utils.DatabasePathFlags),
				Description: `
kaigo snapshot check-dangling-storage <state-root> traverses the snap storage 
...
`,
			...
			// 			},
			{
				...
				Flags:     flags.Merge(utils.NetworkFlags, utils.DatabasePathFlags),
				Description: `
kaigo snapshot traverse-state <state-root>
...
`,
			},
			{
				...
				Flags:     flags.Merge(utils.NetworkFlags, utils.DatabasePathFlags),
				Description: `
kaigo snapshot traverse-rawstate <state-root>
...
`,
			...
			// 			{
			// 				Name:      "dump",
			// 				Usage:     "Dump a specific block from storage (same as 'kaigo dump' but using snapshots)",
			...
			// 				}, utils.NetworkFlags, utils.DatabasePathFlags),
			// 				Description: `
			// This command is semantically equivalent to 'kaigo dump', but uses the snapshots
...
func pruneState(ctx *cli.Context) error {
	...
	prunerconfig := pruner.Config{
		...
		Cachedir:  stack.ResolvePath(config.Kai.TrieCleanCacheJournal),
