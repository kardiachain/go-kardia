...
func TestConsoleWelcome(t *testing.T) {
	...
	gkai.SetTemplateFunc("goarch", func() string { return runtime.GOARCH })
	gkai.SetTemplateFunc("gover", runtime.Version)
	gkai.SetTemplateFunc("gethver", func() string { return configs.VersionWithCommit("", "") })
	gkai.SetTemplateFunc("niltime", func() string {
		return time.Unix(0, 0).Format("Mon Jan 02 2006 15:04:05 GMT-0700 (MST)")
	})
	gkai.SetTemplateFunc("apis", func() string { return ipcAPIs })

	// Verify the actual welcome message to the required template
	gkai.Expect(`
Welcome to the Gkai JavaScript console!

instance: Gkai/v{{gethver}}/{{goos}}-{{goarch}}/{{gover}}
coinbase: {{.Etherbase}}
at block: 0 ({{niltime}})
 datadir: {{.Datadir}}
 modules: {{apis}}
...
func TestAttachWelcome(t *testing.T) {
	...
	if runtime.GOOS == "windows" {
		ipc = `\\.\pipe\gkai` + strconv.Itoa(trulyRandInt(100000, 999999))
	} else {
		ws := tmpdir(t)
		defer os.RemoveAll(ws)
		ipc = filepath.Join(ws, "gkai.ipc")
	}
	...
	t.Run("http", func(t *testing.T) {
		...
		testAttachWelcome(t, gkai, endpoint, httpAPIs)
	})
...
func testAttachWelcome(t *testing.T, gkai *testgeth, endpoint, apis string) {
	// Attach to a running gkai note and terminate immediately
	...
	attach.SetTemplateFunc("etherbase", func() string { return gkai.Etherbase })
