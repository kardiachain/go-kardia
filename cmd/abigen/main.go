...
func init() {
	app = flags.NewApp(gitCommit, gitDate, "kardia checkpoint helper tool")
	app.Flags = []cli.Flag{
		abiFlag,
		binFlag,
		typeFlag,
	...
	app.Action = flags.MigrateFlags(abigen)
...
func abigen(c *cli.Context) error {
	...
	case "objc":
		lang = bind.LangObjC
		flags.Fatalf("Objc binding generation is uncompleted")
	default:
		flags.Fatalf("Unsupported destination language \"%s\" (--lang)", c.GlobalString(langFlag.Name))
	}
	...
	if c.GlobalString(abiFlag.Name) != "" {
		...
		if err != nil {
			flags.Fatalf("Failed to read input ABI: %v", err)
		}
		...
		if binFile := c.GlobalString(binFlag.Name); binFile != "" {
			if bin, err = ioutil.ReadFile(binFile); err != nil {
				flags.Fatalf("Failed to read input bytecode: %v", err)
			}
			if strings.Contains(string(bin), "//") {
				flags.Fatalf("Contract has additional library references, please use other mode(e.g. --combined-json) to catch library infos")
			}
		}
		...
		case c.GlobalIsSet(jsonFlag.Name):
			...
			if err != nil {
				flags.Fatalf("Failed to read combined-json from compiler: %v", err)
			}
			contracts, err = compiler.ParseCombinedJSON(jsonOutput, "", "", "", "")
			if err != nil {
				flags.Fatalf("Failed to read contract information from json output: %v", err)
			}
		}
		...
		for name, contract := range contracts {
			...
			if err != nil {
				flags.Fatalf("Failed to parse ABIs from compiler output: %v", err)
			}
			abis = append(abis, string(abi))
	...
	if err != nil {
		flags.Fatalf("Failed to generate ABI binding: %v", err)
	}
	...
	if err := ioutil.WriteFile(c.GlobalString(outFlag.Name), []byte(code), 0600); err != nil {
		flags.Fatalf("Failed to write ABI binding: %v", err)
	}
	return nil
}
