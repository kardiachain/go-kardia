		...
		for {
			...
			case subres, ok := <-process:
				...
				for _, bloomSinks := range sectionSinks {
					...
					for _, bitSink := range bloomSinks {
						...
						if andVector == nil {
							...
							common.ANDBytes(andVector, andVector, data)
						}
					}
					if orVector == nil {
						orVector = andVector
					} else {
						common.ORBytes(orVector, orVector, andVector)
					}
				}
				...
				if subres.bitset != nil {
					common.ANDBytes(orVector, orVector, subres.bitset)
				}
				if common.TestBytes(orVector) {
					select {
					case <-session.quit:
						return
