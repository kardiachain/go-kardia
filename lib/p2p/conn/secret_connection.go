...
func MakeSecretConnection(conn io.ReadWriteCloser, locPrivKey *ecdsa.PrivateKey) (*SecretConnection, error) {
	var (
		locPubKey = locPrivKey.PublicKey
	)
...
func (sc *SecretConnection) RemotePubKey() ecdsa.PublicKey {
	return sc.remPubKey
}
...
func signChallenge(challenge *[32]byte, locPrivKey *ecdsa.PrivateKey) ([]byte, error) {
	signature, err := crypto.Sign(challenge[:], locPrivKey)
	if err != nil {
		return nil, err
	}
...
func shareAuthSignature(sc io.ReadWriter, pubKey ecdsa.PublicKey, signature []byte) (recvMsg authSigMessage, err error) {
