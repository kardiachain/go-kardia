...
func TestNewSimulatedBackend(t *testing.T) {
	...
	if sim.config != configs.AllEthashProtocolChanges {
		t.Errorf("expected sim config to equal configs.AllEthashProtocolChanges, got %v", sim.config)
	}

	if sim.blockchain.Config() != configs.AllEthashProtocolChanges {
		t.Errorf("expected sim blockchain config to equal configs.AllEthashProtocolChanges, got %v", sim.config)
	}
...
func TestNewSimulatedBackend_AdjustTimeFail(t *testing.T) {
	...
	sim := simTestBackend(testAddr)
	// Create tx and send
	tx := types.NewTransaction(0, testAddr, big.NewInt(1000), configs.TxGas, big.NewInt(1), nil)
	...
	tx2 := types.NewTransaction(1, testAddr, big.NewInt(1000), configs.TxGas, big.NewInt(1), nil)
...
func TestSimulatedBackend_NonceAt(t *testing.T) {
	...
	tx := types.NewTransaction(nonce, testAddr, big.NewInt(1000), configs.TxGas, big.NewInt(1), nil)
	signedTx, err := types.SignTx(tx, types.HomesteadSigner{}, testKey)
	if err != nil {
		t.Errorf("could not sign tx: %v", err)
	}
...
func TestSimulatedBackend_SendTransaction(t *testing.T) {
	...
	// create a signed transaction to send
	tx := types.NewTransaction(uint64(0), testAddr, big.NewInt(1000), configs.TxGas, big.NewInt(1), nil)
	signedTx, err := types.SignTx(tx, types.HomesteadSigner{}, testKey)
	if err != nil {
		t.Errorf("could not sign tx: %v", err)
	}
...
func TestSimulatedBackend_EstimateGas(t *testing.T) {
	...
	sim := NewSimulatedBackend(core.GenesisAlloc{addr: {Balance: big.NewInt(configs.Ether)}}, 10000000)
	defer sim.Close()
...
func TestSimulatedBackend_EstimateGasWithPrice(t *testing.T) {
	...
	sim := NewSimulatedBackend(core.GenesisAlloc{addr: {Balance: big.NewInt(configs.Ether*2 + 2e17)}}, 10000000)
	defer sim.Close()
...
func TestSimulatedBackend_PendingNonceAt(t *testing.T) {
	...
	tx = types.NewTransaction(uint64(1), testAddr, big.NewInt(1000), configs.TxGas, big.NewInt(1), nil)
	signedTx, err = types.SignTx(tx, types.HomesteadSigner{}, testKey)
	if err != nil {
		t.Errorf("could not sign tx: %v", err)
	}
